...
public extension UIViewController {
     ...
     */
    public func setMenuFilter (sender: UIButton) {
        
        if(!(self as! MDProductsViewController).isShowOnBoard)
        {
            sender.isSelected = !sender.isSelected
            sideMenuController()?.sideMenu?.setMenuController(menuViewController: MDFilterMenuViewController())
            sideMenuController()?.sideMenu?.menuWidth = UIScreen.main.bounds.width/3
            sideMenuController()?.sideMenu?.toggleMenu()
            if ((sideMenuController()?.sideMenu?.isMenuOpen)! == false)
            {
                for case let button as UIButton in (self.navigationItem.rightBarButtonItem?.customView?.subviews)! {
                    if(button.tag != 1)
                    {
            ...
            {
                (self as! MDProductsViewController).isFilter = true
                for case let button as UIButton in (self.navigationItem.rightBarButtonItem?.customView?.subviews)! {
                    if(button.tag != 1)
                    {
        ...
        {
            (self as! MDProductsViewController).changePage()
        }
    }
    
    public func setMenuShoppingCart(sender: UIButton) {
        (sender as! MIBadgeButton).badgeString = String(MDShoppingCartViewController.shoppingCart.products.count)
        if(MDShoppingCartViewController.shoppingCart.products.count == 0)
        {
            (sender as! MIBadgeButton).badgeLabel.isHidden = true
        }
        else
        {
            (sender as! MIBadgeButton).badgeLabel.isHidden = false
        }
        
        sideMenuController()?.sideMenu?.setMenuController(menuViewController: MDShoppingCartViewController())
        sideMenuController()?.sideMenu?.menuWidth = UIScreen.main.bounds.width/2
        sideMenuController()?.sideMenu?.toggleMenu()
        if ((sideMenuController()?.sideMenu?.isMenuOpen)! == false)
        {
            for case let button as UIButton in (self.navigationItem.rightBarButtonItem?.customView?.subviews)! {
    ...
    public func hideSideMenuView () {
        ...
        if((self.navigationItem.rightBarButtonItem?.customView?.subviews) != nil)
        {
            for case let button as UIButton in (self.navigationItem.rightBarButtonItem?.customView?.subviews)! {
                button.isEnabled = true
    ...
    func closeSesion(sender: UILongPressGestureRecognizer)
    {
        if (sender.state == .began){
            
            MDSession.logout { (hasError: Bool) in
                if(hasError == false)
                {
...
public class ENSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    public func setMenuController (menuViewController: UIViewController) {
        for v in sideMenuContainerView.subviews{
    ...
    private func setupMenuView() {
        ...
        updateFrame()
        sideMenuContainerView.backgroundColor = UIColor.white
